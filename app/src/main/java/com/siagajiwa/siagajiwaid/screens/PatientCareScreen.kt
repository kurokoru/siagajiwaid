...
fun PatientCareScreen(
    ...
    val mediaState by viewModel.patientCareMediaState.collectAsState()

    // Load media when screen is first displayed
    LaunchedEffect(Unit) {
        viewModel.loadPatientCareMedia()
    }
            ...
            when (val state = mediaState) {
                    ...
                    LazyColumn(
                        ...
                        items(state.media) { mediaContent ->
                            AsyncImage(
                                ...
                                contentDescription = "Patient Care Material ${mediaContent.order}",
                                modifier = Modifier
                                    .fillMaxWidth()
                ...
                is MediaUiState.Error -> {
                    ErrorContent(
                        ...
                        onRetry = { viewModel.retryLoading(MediaType.PATIENT_CARE) }
                    )
                }
            }
        }
