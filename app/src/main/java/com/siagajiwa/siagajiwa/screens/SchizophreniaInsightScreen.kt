...
import com.siagajiwa.siagajiwa.viewmodel.MediaType
import com.siagajiwa.siagajiwa.viewmodel.MediaUiState
import com.siagajiwa.siagajiwa.viewmodel.MediaViewModel

@Composable
fun SchizophreniaInsightScreen(
    navController: NavHostController,
    viewModel: MediaViewModel = viewModel()
) {
    val mediaState by viewModel.schizophreniaMediaState.collectAsState()
    var selectedBottomNavIndex by remember { mutableIntStateOf(0) }

    // Load media when screen is first displayed
    LaunchedEffect(Unit) {
        viewModel.loadSchizophreniaMedia()
    }
    ...
    Box(
        ...
        Column(
            ...
            SchizophreniaInsightNavigationBar(
                title = "Selayang Pandang Gangguan Jiwa",
            ...
            when (val state = mediaState) {
                    ...
                    LazyColumn(
                        ...
                        items(state.media) { mediaContent ->
                            AsyncImage(
                                ...
                                contentDescription = "Schizophrenia Insight Material ${mediaContent.order}",
                                modifier = Modifier
                                    .fillMaxWidth()
                ...
                is MediaUiState.Error -> {
                    ErrorContent(
                        ...
                        onRetry = { viewModel.retryLoading(MediaType.SCHIZOPHRENIA) }
                    )
                }
            }
        }
